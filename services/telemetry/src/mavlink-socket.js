...
export default class MavlinkSocket extends EventEmitter {
    ...
    // Emit a receive event for each message, and an event specific
    // to each type.
    _registerMessageListener() {
        this._mav.on('message', (message) => {
            let type = this._mav.getMessageName(message.id);

            // Wait for specific message event to get the all the
            // fields.
            this._mav.once(type, (_, fields) => {
                ...
                // Emit another event if the message was not listened
                // for.
                if (!listened) {
